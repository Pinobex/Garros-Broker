...
const BROKER_ADDRESS: &str = "0.0.0.0";
const PORT: u16 = 1884;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut mqttoptions = MqttOptions::new("test", BROKER_ADDRESS, PORT);
...
async fn requests(client: AsyncClient) {
     ...
     */
    client
        .subscribe(TOPIC, mqttbytes::QoS::AtMostOnce)
        .await
        .unwrap();
    ...
    for i in 1..=10 {
        client
            .publish(TOPIC, mqttbytes::QoS::ExactlyOnce, false, vec![1; i]) 
            .await
            .unwrap();
